...
class Buku extends CI_Controller {
	...
	public function buku()
	{
		$data['data_buku'] = $this->Buku_model->getAllData("tb_buku");
			...
			$this->load->view('buku/R_buku',$data,$tmp);
	...
	public function hapus_buku()
	{
		$id_buku = $this->input->get('id_buku', TRUE);			
			...
			header('location:'.base_url().'index.php/admin/data_buku');
	}
	...
	public function tambah_buku()
	{
		$this->form_validation->set_rules('id_buku', 'id_buku', 'required');
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				$tmp['content']=$this->load->view('layout');
				$this->load->view('buku/C_buku',$data,$tmp);
			...
			else
			{
				...
				if($isi==0){
					$data= array (
							...
							redirect ('index.php/admin/data_buku');
				}
				else 
					{
					/*layout*/	
	...
	public function edit_buku()
	{
		$id_buku = $this->input->get('id_buku', TRUE);	
			...
			$data['data_kategori'] = $this->Buku_model->getAllData("tb_kategori");
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('layout');
				$this->load->view('buku/U_buku',$data,$tmp);
	...
	public function update_buku()
	{
		$this->form_validation->set_rules('judul', 'judul', 'required');
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('buku/U_buku',$data, TRUE);
			...
			else
			{
				...
				redirect('index.php/admin/data_buku','refresh');
	...
	public function detail_stok(){
		$id_buku = $this->input->get('id_buku', TRUE);	
			...
			/*masukan data kedalam view */
			$tmp['content']=$this->load->view('layout');
			$this->load->view('buku/R_detail_stok',$data, $tmp);
	...
	public function hapus_det_buku()
	{
		$id_buku = $this->input->get('id_buku',TRUE);		
			...
			header('location:'.base_url().'index.php/buku/detail_stok/?id_buku='.$id_buku.'');
			}else{
				//tampilkan error
	...
	public function tambah_det_buku()
	{
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('layout');
				$this->load->view('buku/C_detail_stok',$data, $tmp);
			...
			else
			{
		            ...
		            header('location:'.base_url().'index.php/buku/tambah_det_buku/?id_buku='.$id_buku.'');
					}
					else{
	...
	public function edit_det_buku()
	{
		$id_det_buku = $this->input->get('id_detail_buku', TRUE);	
			...
			/*layout*/	
				...
				/*masukan data kedalam view */
				$tmp['content']=$this->load->view('layout');
				$this->load->view('buku/U_detail_stok',$data, $tmp);
