...
if ($result = $con->query("select judul from tb_buku", MYSQLI_USE_RESULT)) {
...
if ($result = $con->query("select group_concat(tb_buku.judul separator ',')
    from apriori left join tb_buku 
	 on (apriori.id_buku = tb_buku.id_buku)
	 group by apriori.nama", MYSQLI_USE_RESULT)) {
...
<?php
    ...
    for($i = 0; $i < $item1; $i++) {
        for($j = $i+1; $j < $item2; $j++) {
          ...
          echo " <tr> <td colspan='3'>Tidak di temukan item gabungan 2</td></tr>";
        }
        }
    }
	echo"</table>";
?>
  ...
  echo"<tr> <td>Item Set</td><td> Support Count</td><td>Support</td></tr>";
    ...
    if ($item3<3) {
   echo " <tr> <td colspan='3'>Tidak di temukan item gabungan 3</td></tr>";
   ...
   echo " <tr> <td colspan='3'>Tidak di temukan item gabungan 3</td></tr>";
    ...
    foreach ($item_array as $ia_key => $ia_value) {
      ...
      if($lift_ratio<5&&$in_percent>50&&$in_percent!=100){
          ...
          echo " <tr> <td colspan='3'>Tidak di temukan item confidence > 50</td></tr>";
        }
